...
float   acceleration_values[6]      = {0.6f, 0.8f, 1.0f, 1.5f, 1.7777777f, 2.0f};
...
float   linear_reduction_values[6]  = {0.6f, 0.7f, 0.75f, 0.8f, 1.0f, 2.0f};
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [_FUNCTION] =
    { /*FUNCTION*/
        { QK_BOOT,  XXXXXXX,  XXXXXXX,  XXXXXXX,  XXXXXXX,  XXXXXXX,         XXXXXXX,  XXXXXXX, A_B_TEST, OVERVIEW,  QK_BOOT,  TO(_SETTINGS) },
        ...
        switch (current_layer) {
            case _QWERTY:
                ...
                if ( is_caps_word_on() ) {
                    ...
                    oled_write_ln_P(PSTR("     Colemak-DH     "), true);
                    status = 4;
                }
                if (current_layer == prev_layer && status == prev_status) {
                    break;
                }
                prev_status = status;
#if 1
